...
import { serveHttp } from "breeze-js/runtime";
    ...
    <html lang="en">
      ...
      <body class="py-5">
        <div class="flex flex-col items-center gap-3 w-full">
          <form action="${buildUrl("/objects")}" method="post" enctype="multipart/form-data" class="flex flex-col items-center gap-2">
            <label for="uploader">
...
app.post("/objects", async (ctx) => {
	...
	const objectsEndpoint = await pluginInstance.getEndpoint("/objects");

	const requestUploadRes = await fetch(objectsEndpoint, {
		method: "POST",
	...
	return ctx.redirect(getBaseUrl());
});

app.get("/objects/:id", async (ctx) => {
	const id = ctx.req.param("id");
	const objectEndpoint = await pluginInstance.getEndpoint(`/objects/${id}`);
	const objectRes = await fetch(objectEndpoint, { method: "GET" });
...
app.delete("/objects/:id", async (ctx) => {
	const id = ctx.req.param("id");
	const objectEndpoint = await pluginInstance.getEndpoint(`/objects/${id}`);
	const res = await fetch(objectEndpoint, { method: "DELETE" });
